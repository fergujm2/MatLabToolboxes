function [p, R, n, u] = UnpackRodState(state)
p = state(1:3,1);
R = [state(4:6), state(7:9), state(10:12)];
n = state(13:15);
u = state(16:18);

% Technically, it's better to orthogonalize R every time we unpack
% R = orthogonalize(R);
end

function R = orthogonalize(R0)
